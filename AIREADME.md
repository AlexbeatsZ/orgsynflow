...
- [done] 2026-06-20 修复“3D 构象预览 (类似 GaussView)”不可用：3Dmol 改为前端本地依赖并统一加载，viewer 高度不再被 flex 压缩；补齐每个子模型的标准 XYZ 头，球棍结构正常显示，原子编号默认关闭。交互固定为普通左键拖拽视角、Shift+左键拖拽选中分子、Ctrl+左键点击选择分子，另保留滚轮缩放、方向键微调、逐分子 XYZ 旋转、重置/居中和可选原子编号。预览坐标与 GJF 输入联动。浏览器回归确认普通左键只改变画面像素而 GJF 不变；Shift 拖拽后 X/Y 由 0/0 变为 1.2/-0.4 且 GJF 改变；Ctrl 点击从分子 1 切换到分子 2；`npm run build` 成功。
