...
- Do not package heuristic logic as authentic model results.
...
- The default workspace file `data/workspaces/example-workspace.json` may contain cached results (e.g., status/used_fallback records in `molecule:mol-ethanol:retrosynthesis`), causing the page to display outdated status on initial load. Such task result caches must be manually cleaned to restore the initial "uncomputed" state, allowing users to run real retrosynthesis calculations.
...
- [todo] If a real ML yield layer is needed, prioritize evaluating `rxn4chemistry/rxn_yields` or other niche-domain public models, enforcing UI display of training sources, reaction families/applicable domains, and uncertainties.
...
- 2026-06-20 Rollback to Chinese UI & Backend: Reverted the uncommitted English translations of backend modules (`core/properties.py`, `core/reaction_explain.py`, `core/yield_predictor.py`, `core/ts_workflow.py`, `app/eas_energy_plot.py`, `app/main.py`) to the Chinese version at HEAD. Restored the frontend interface (`web/src/App.tsx`) to the Chinese version from `0a8f7382` while preserving the layout reset fix and orthogonal routing performance optimizations introduced in `99c42951`. Rebuilt the frontend successfully using `npm run build` and restarted all services using `scripts/orgsynflow-toggle.ps1`. All 55 backend tests passed.
...
- 2026-06-21 CREST slow-job diagnosis: job `cd6fb7db` was not performing computation despite being reported as `running`. No OrgSynFlow `wsl.exe`, `crest`, or `xtb` process existed, and `/tmp/codex/orgsynflow/crest_jobs/20260621_002309_619142-01e34d67` did not exist. The stale status was reproduced after the API restart and traced to an uncommitted hard-coded job restoration block in `CrestJobManager.__init__` (`core/job_queue.py`). No computation or source-code fix was performed during this read-only diagnosis; remove the hard-coded restoration and mark interrupted jobs terminal before treating the queue status as authoritative.
- 2026-06-21 CREST stale-job fix: removed the hard-coded `cd6fb7db` restoration from `CrestJobManager`, so a restarted backend now starts with an empty in-memory CREST queue and the frontend's interrupted-task recovery marks missing jobs terminal instead of leaving them `running`. Preserved and validated live CREST work-directory/log progress support, added regression coverage for phantom-job prevention and CREST energy parsing, and restarted the local services. Verification: 42 non-external Python tests passed, focused CREST tests passed 9, frontend build succeeded, a real water CREST 3.0.2 search completed in about 6.8 seconds with `lowest_conformer_xyz`, and forced cancellation of a real dodecane search left no WSL CREST process.
- 2026-06-21 CREST live-log and ETA fix: the backend already exposed growing `log_tail`/`log_progress`, but `syncGaussianTaskRecords()` skipped updates whenever both snapshots remained `running`, and an open result modal retained its original payload. The WSL CREST command also redirected stdout without line buffering, allowing the live log file to remain unchanged during a long MTD stage. Running snapshots are now compared by log/progress/result content, open job modals follow the polled job snapshot, CREST launches through `stdbuf -oL -eL`, and trial-MTD estimates are parsed and displayed as single-MTD and batch durations with an advisory note. The active bromobenzene job `007d001a` remained CPU-active with no errors and reported a 17-minute-57-second batch estimate; it was not interrupted, so line buffering applies from the next CREST launch. Verification: focused CREST tests passed 9, frontend production build succeeded, WSL GNU `stdbuf` 9.4 is available, and the current workspace record received the live 4520-character log plus three energy steps.
